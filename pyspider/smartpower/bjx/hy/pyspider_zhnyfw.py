...
class Handler(BaseHandler):
    ...
    def __init__(self):
        self.spider_config = {
            "source_url": 'https://news.bjx.com.cn/search',
            'urls': [
                {'url': '/?type=5&kw=%E7%BB%BC%E5%90%88%E8%83%BD%E6%BA%90%E6%9C%8D%E5%8A%A1', 'classify': 'zhnyfw', 'classify_name': u'综合能源服务', 'industry': u'综合能源服务'},
            ],
            'db': {
                'host': '39.98.39.58',
                'user': 'root',
    ...
    def next_page(self, response):
        ...
        for i in range(2, int(pageCount) + 1):
            self.crawl(response.url + '&index=' + str(i),
    ...
    def item_page(self, response):
        boxs = response.doc('.cc-list > .item > .top > span > a').items()
        for box in boxs:
    ...
    def detail_page(self, response):
        title = response.doc('.cc-headline > .box > h1')
        if not title:
            return
