...
class Handler(BaseHandler):
    ...
    def __init__(self):
        self.spider_config = {
            "source_url": 'http://cqbiocity.com/ml/pc/news/pc_news.html',
            'urls': [
                {'url': '?columnsCode=00030001&isAjax=true&rtime=1612098547961&isLoad=true', 'classify': 'news', 'classify_name': u'园区动态'},
                {'url': '?columnsCode=00030002&isAjax=true&rtime=1612098583446&isLoad=true', 'classify': 'news', 'classify_name': u'企业新闻'},
                {'url': '?columnsCode=00030003&isAjax=true&rtime=1612098583446&isLoad=true', 'classify': 'news', 'classify_name': u'媒体报道'},
                {'url': '?columnsCode=00030005&isAjax=true&rtime=1612098583446&isLoad=true', 'classify': 'news', 'classify_name': u'通知公告'},
                {'url': '?columnsCode=0008&isAjax=true&rtime=1612098583446&isLoad=true', 'classify': 'news', 'classify_name': u'党建信息'},
            ],
            'db': {
                'host': '192.168.0.103',
                'user': 'root',
    ...
    def next_page(self, response):
        ...
        # 继续爬取下一页列表
        next_page = response.doc('ul.pagination > li > .next').attr('pageno')
    ...
    def item_page(self, response):
        boxs = response.doc('#news_list > .news_list').items()
        for box in boxs:
            pub_date = box('.news_list_time').text().strip()
    ...
    def detail_page(self, response):
        title = response.doc('#news > #detail > .detail_title').text()
        content = response.doc('#news > #detail > .detail_content').html()
