...
class Handler(BaseHandler):
    ...
    def __init__(self):
        self.spider_config = {
            "source_url": 'http://www.bioasis.cn/window',
            'urls': [
                {'url': '/ruzhujigou.jsp', 'classify': 'enterprise', 'classify_name': u'入驻机构'},
                {'url': '/ruzhuqiye.jsp', 'classify': 'enterprise', 'classify_name': u'入驻企业'},
            ],
            'db': {
                'host': '192.168.0.103',
                'user': 'root',
    ...
    def item_page(self, response):
        boxs = response.doc('.companyList > li').items()
        for box in boxs:
            art_href = box('a').attr('href')
            self.crawl(art_href, validate_cert=False,
                       save={'classify_name': response.save['classify_name'],
                             'classify': response.save['classify']}, callback=self.detail_page)

    @config(priority=2)
    def detail_page(self, response):
        title = response.doc('.midContent > .title').text().strip()
        content = response.doc('.midContent > .listcontent').html().strip()
        ret = {
