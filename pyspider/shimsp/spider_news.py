...
class Handler(BaseHandler):
    ...
    def __init__(self):
        self.spider_config = {
            "source_url": 'https://sh.zhaoshang.net/yuanqu/detail/919/newslist',
            'urls': [
                {'url': '/8/', 'classify': 'news', 'classify_name': u'园区动态'},
                {'url': '/7', 'classify': 'policy', 'classify_name': u'优惠政策'}
            ],
            'db': {
                'host': '192.168.1.14',
                'user': 'root',
    ...
    def on_start(self):
        for item in self.spider_config['urls']:
            ...
            self.crawl(self.spider_config['source_url'] + item['url'], save=m_params, fetch_type="js",
    ...
    def detail_page(self, response):
        ...
        brothers = response.doc('.related-content').parent().children().items()
        flag = False
        content = None
        for bro in brothers:
            if flag:
                content = bro.html().strip()
                break
            if bro('.related-content').html() is not None:
                flag = True
        if content is None:
            return
        ret = {
